...
if(isset($_POST['add_to_cart'])){
  ...
  //query
  $select_cart = mysqli_query($con, "SELECT * FROM carrito WHERE id_producto = '$id_producto' AND id_usuario = '$user'") or die('query failed');

  header("Location:carrito.php");
  //verificar que regreso alguna información
  if(mysqli_num_rows($select_cart) > 0){
     $message[] = '¡Producto ya esta en el carrito!';
  }else{
     mysqli_query($con, "INSERT INTO carrito (id_usuario, id_producto, id_inventario, cantidad, subtotal) VALUES('$user', '$id_producto', '$id_inv', '$cantidad', ($precio*$cantidad))") or die('query failed');
